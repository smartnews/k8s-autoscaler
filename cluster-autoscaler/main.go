...
var (
	...
	podNamespace               = flag.String("pod-namespace", apiv1.NamespaceAll, "If set, ClusterAutoscaler will only look at pods under the namespace, otherwise it looks at all pods")
	...
	expanderPriorityConfigMapName = flag.String(
		"expander-priority-configmap", "cluster-autoscaler-priority-expander",
		"a name of the ConfigMap used to store priority expander configuration")
...
func createAutoscalingOptions() config.AutoscalingOptions {
	...
	return config.AutoscalingOptions{
		...
		ExpanderPriorityConfigMapName:    *expanderPriorityConfigMapName,
...
func buildAutoscaler(debuggingSnapshotter debuggingsnapshot.DebuggingSnapshotter) (core.Autoscaler, error) {
	...
	informerFactory := informers.NewSharedInformerFactoryWithOptions(kubeClient, 0, informers.WithTransform(trim), informers.WithNamespace(autoscalingOptions.PodNamespace))
